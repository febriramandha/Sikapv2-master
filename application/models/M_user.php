...
class M_user extends CI_Model {
	...
	{
		$this->db->select('a.id as user_id, b.id as login_id, b.username, a.nama, a.nip, a.tpp, a.dept_id, b.level, a.att_status, a.pns, b.status, c.gelar_dpn, c.gelar_blk, c.jabatan, c.golongan_id, c.eselon_id, c.gender, c.agama_id,c.statpeg_id, a.absen_online_app,a.simpeg_pegawai_id,j.id as eselon_simpeg_id, j.eselon as eselon_simpeg, k.id as golongan_simpeg_id, k.golongan as golongan_simpeg, g.nama_jabatan as jabatan_simpeg, l.id as kelas_jabatan_id, l.kelas_jabatan, e.gelar_depan as gelar_depan_simpeg, e.gelar_blkng as gelar_blkng_simpeg, e.nama_pegawai as nama_pegawai_simpeg')
				 ->from('mf_users a')
				 ->join('users_login b','a.id=b.user_id')
         ->join('sp_pegawai c','a.id=c.user_id','left')
         ->join(''.$this->data['db_connect'].'.pegawai e','a.simpeg_pegawai_id = e.id OR a.nip = e.nip','left')
         ->join(''.$this->data['db_connect'].'.detail_jabatan f','e.detail_jabatan_id = f.id','left')
         ->join(''.$this->data['db_connect'].'.jabatan g','f.jabatan_id = g.id','left')
         ->join(''.$this->data['db_connect'].'._eselon h','g.eselon_id = h.id','left')
         ->join("(SELECT * FROM ".$this->data['db_connect'].".riwayat_golongan WHERE status='1') i",'e.id = i.pegawai_id','left')
         ->join('_eselon j','h.id = j.simpeg_id_eselon','left')
         ->join('_golongan k','i.golongan_id = k.golongan_id_simpeg','left')
         ->join(''.$this->data['db_connect'].'.kelas_jabatan l','g.kelas_jabatan_id = l.id','left')
         ->where('a.id', $user_id);
		return $this->db->get();
	}
  ...
  public function GetSyncPegawai($id){
    ...
    $this->db->join(''.$this->data['db_connect'].'.pegawai b','b.nip= a.nip');
    $this->db->join(''.$this->data['db_connect'].'.detail_jabatan c','c.id = b.detail_jabatan_id');
    $this->db->join(''.$this->data['db_connect'].'.unor d','d.id = c.unor_id');
    $this->db->join('mf_departments e','e.id = a.dept_id');
    $this->db->join(''.$this->data['db_connect'].'.kamus_data f','f.id = b.status_pegawai');
    $this->db->join(''.$this->data['db_connect'].'.users g','g.pegawai_id = b.id');
    $this->db->join(''.$this->data['db_connect'].'.kamus_data h','h.id = b.agama','LEFT');
    $this->db->join(''.$this->data['db_connect'].'.jabatan i','i.id = c.jabatan_id');
    $this->db->join(''.$this->data['db_connect'].'._pejabat_level j','j.id = i.pejabat_level_id');
    $this->db->join(''.$this->data['db_connect'].'.riwayat_golongan k','k.pegawai_id = b.id');
    $this->db->join(''.$this->data['db_connect'].'.golongan l','l.id = k.golongan_id');
    $this->db->join(''.$this->data['db_connect'].'._eselon m','m.id = i.eselon_id');
    ...
    $this->db->join(''.$this->data['db_connect'].'.kelas_jabatan r','r.id = i.kelas_jabatan_id','LEFT');
  ...
  public function getSimpegDeptUser($dept_id)
  {
    ...
    $this->db->join(''.$this->data['db_connect'].'.unor b','a.simpeg_dept_id = b.id');
    $this->db->join(''.$this->data['db_connect'].'.detail_jabatan c','b.id = c.unor_id');
    $this->db->join(''.$this->data['db_connect'].'.pegawai d','c.id = d.detail_jabatan_id');
    $this->db->join(''.$this->data['db_connect'].'.kamus_data e','d.status_pegawai = e.id');
   ...
   public function GetSyncPegawaiNon($id){
    $this->db->select('a.id, b.nip,b.nama_pegawai,d.nama_unor,d.id AS unor_id,e.dept_name AS nama_unor_sikap,e.id AS sikap_dept_id,ee.id AS sikap_dept_id_pindah,ee.dept_name AS sikap_dept_pindah,f.type_kamus AS status_pegawai,g.username AS username_simpeg,g.password AS password_simpeg,h.kamus_data AS agama,h.id AS agama_id,i.nama_jabatan,j.id AS pejabat_id,j.nama_pejabat AS status_jabatan_simpeg,b.gelar_depan AS glr_dpn_simpeg,b.gelar_blkng AS glr_blkng_simpeg,b.jenis_kelamin,b.tgl_lahir,l.id AS golongan_id,l.nama_golongan,l.ruang,l.pangkat,k.status,m.id AS eselon_id_simpeg,m.nama_eselon,g.status AS status_akun_simpeg, n.id AS agama_id_sikap, n.agama AS agama_sikap, o.id AS status_pegawai_sikap_id, o.nama AS status_pegawai_sikap,p.id as golongan_id_sikap, q.id as eselon_id_sikap, r.id as kelas_id_jabatan, r.id as kelas_jabatan_id,s.level as level_sikap');
    $this->db->join(''.$this->data['db_connect'].'.pegawai b','b.id= a.simpeg_pegawai_id');
    $this->db->join(''.$this->data['db_connect'].'.detail_jabatan c','c.id = b.detail_jabatan_id');
    $this->db->join(''.$this->data['db_connect'].'.unor d','d.id = c.unor_id');
    $this->db->join('mf_departments e','e.id = a.dept_id');
    $this->db->join(''.$this->data['db_connect'].'.kamus_data f','f.id = b.status_pegawai');
