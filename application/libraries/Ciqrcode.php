	...
	public function generate($params = array()) {
		...
		if (isset($params['savename'])) {
			...
			if (isset($params['logo'])) {
				$generatedQR = imagecreatefrompng($params['savename']);
				$logo = imagecreatefromstring(file_get_contents($params['logo']));

				imagecolortransparent($logo , imagecolorallocatealpha($logo , 255, 255, 255, 127));
				imagealphablending($logo , false);
				imagesavealpha($logo , true);

				$QR_width = imagesx($generatedQR);
				$QR_height = imagesy($generatedQR);
				...
				$logo_size = 5;
				if (isset($params['logo_size'])) $logo_size = min(max((int)$params['logo_size'], 1), 10);
				$logo_x_position = $logo_size/2;
				if (isset($params['logo_position_x'])) $logo_x_position = min(max((float)$params['logo_position_x'], 1), 10);
				$logo_y_position = $logo_size/2;
				if (isset($params['logo_position_y'])) $logo_y_position = min(max((float)$params['logo_position_y'], 1), 10);

				$logo_qr_width = $QR_width/$logo_size;
				$scale = $logo_width/$logo_qr_width;
				$logo_qr_height = $logo_height/$scale;

				$out = imagecreatetruecolor($QR_width, $QR_width);
				imagecopyresampled($out, $generatedQR, 0, 0, 0, 0, $QR_width, $QR_height, $QR_width, $QR_height);
				imagecopyresampled($out, $logo, $QR_width/$logo_x_position, $QR_height/$logo_y_position, 0, 0, $logo_qr_width, $logo_qr_height, $logo_width, $logo_height);
				imagepng($out,$params['savename']);
			}
			return $params['savename'];
		} else {
			if(empty($params['tempname'])){$params['tempname'] = 'temp-file.png';}
			$tempdir = './assets/temp/';
			...
			QRcode::png($params['data'], $tempdir.$params['tempname'] , $level, $size, 2);

			if (isset($params['logo'])) {
				$generatedQR = imagecreatefrompng($tempdir.$params['tempname']);
				$logo = imagecreatefromstring(file_get_contents($params['logo']));
				...
				imagecopyresampled($out, $generatedQR, 0, 0, 0, 0, $QR_width, $QR_height, $QR_width, $QR_height);
				imagecopyresampled($out, $logo, $QR_width/$logo_x_position, $QR_height/$logo_y_position, 0, 0, $logo_qr_width, $logo_qr_height, $logo_width, $logo_height);
				imagepng($out, $tempdir.$params['tempname']);
			}

			$read = file_get_contents($tempdir.$params['tempname']);
			unlink($tempdir.$params['tempname']);
