...
<div class="card">
	<div class="card-header bg-white header-elements-inline py-2">
		<h5 class="card-title">Absen Wirid</h5>
	...
	<div class="card-body">
		<?php echo form_open('kepegawaian/absen-wirid/AjaxSave/'.$this->uri->segment(4),'class="form-horizontal" id="formAjax"'); ?>
		<div class="form-group row">
			...
			<div class="col-lg-10">
				<div class="form-group-feedback form-group-feedback-left">
					...
					<input type="text" class="form-control" value="<?php echo tgl_ind_hari($jadwal->tgl_apel).' '.jm($jadwal->start_time) ?>" disabled>
				</div>
			</div>
		</div>
		<div class="form-group row">
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		table = $('#datatable').DataTable({
			...
			ajax: {
				url : uri_dasar+'kepegawaian/absen-wirid/PegawaiJson/<?php echo $this->uri->segment(4) ?>',
				type:"post",
				"data": function ( data ) {
	...
	function confirmAksi(id) {
		$.ajax({
			url: uri_dasar+'kepegawaian/absen-wirid/AjaxDel',
			data: {id: id},
			dataType :"json",
			error:function(){
				bx_alert('gagal menghubungkan ke server cobalah mengulang halaman ini kembali');
	...
	$('#cetak').click(function() {
		newWindow = window.open(uri_dasar + 'kepegawaian/absen-wirid/cetak/'+$('[name="instansi"]').val()+'/<?php echo $this->uri->segment(4) ?>',"open",'height=600,width=800');
