	...
	<div class="card-body">
		...
		<div class="col-lg-12">
          <div class="form-group row">
              ...
              <div class="col-lg-10">
                  <div class="form-group-feedback form-group-feedback-left">
                      ...
                      <input type="text" name="tanggal" class="form-control datepicker" placeholder="tanggal Dinas Luar" value="<?php echo format_tgl_ind($dl_manual->tanggal) ?>">
                  </div>
              </div>
          </div>
          <div class="form-group row">
                  ...
                  <div class="form-group">
                       <select class="form-control multiselect-clickable-groups" name="user[]" multiple="multiple" id="filter_list_dropdwn" data-fouc>
                             <?php foreach ($user as $row) { 
                                    $user_cek =  pg_to_array($dl_manual->user_id);
                                     $selected ='';
                                     for ($i=0; $i < count($user_cek); $i++) { 
                                            if ($user_cek[$i] == $row->id) {
                                                    $selected = "selected";
                                            }
                                     }
                              ?>
                                <option value="<?php echo encrypt_url($row->id,'user_id_dl') ?>" <?php echo $selected ?>><?php echo nama_gelar($row->nama,$row->gelar_dpn, $row->gelar_blk) ?>(<?php echo $row->nip ?>)</option>
                              <?php } ?>
                        </select>
                  </div>
              </div>
           ...
           <div class="form-group row">
              ...
              <div class="col-lg-9">
                 <textarea class="ckeditor" id="edit1" name="kegiatan"><?php echo $dl_manual->uraian ?></textarea>
              </div>
           </div>
            <div class="form-group row">
              <label class="col-lg-2 col-form-label">Hasil <span class="text-danger">*</span></label>
              <div class="col-lg-9">
                 <textarea class="ckeditor" id="edit2" name="hasil"><?php echo $dl_manual->hasil ?></textarea>
              </div>
           </div>
          <input type="hidden" name="id" value="<?php echo encrypt_url($dl_manual->id,'dl_id') ?>">
          ...
          <div class="text-left offset-lg-2" >
              ...
              <button type="submit" class="btn btn-sm btn-info result" id="result">Simpan <i class="icon-checkmark4 ml-2"></i></button>
