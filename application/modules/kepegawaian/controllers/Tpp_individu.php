...
class Tpp_individu extends App_Controller
{
	...
	public function __construct()
	{
		parent::__construct();
		$this->_init();
		$this->breadcrumbs->push('Rekapitulasi Penerimaan TPP', 'kepegawaian/tpp_individu');
	...
	public function index()
	{
		...
		$this->load->view('tpp_individu/v_index', $this->data);
	...
	public function add()
	{
		...
		$this->load->view('tpp_individu/v_add', $this->data);
	}
	...
	public function AjaxGet()
	{
		...
		$dept_id = $this->session->userdata('tpp_dept_id');
       	$this->datatables->select('a.id, a.id_pegawai, a.bbebankerja, a.bkondisikerja, a.bkelangkaan, a.totaltpp, a.potongan_disiplin, a.potongan_produktivitas, a.hasiltpp, b.nama, b.nip, (c.disiplin_kerja/100 * a.totaltpp) as disiplin_kerja, (c.produktivitas_kerja/100 * a.totaltpp) as produktivitas_kerja, a.potonganpph, a.potonganbpjs, a.setelahpotongpph, a.jml_setelah_potongbpjs, a.potonganzakat, a.jml_setelah_potongzakat')
			->from('pegawai_tpp a')
			->join('mf_users b', 'a.id_pegawai=b.id')
			->join('tpp_standar c', 'a.tpp_standar=c.id')
			->where('b.att_status', 1)
			->where('b.dept_id', $dept_id)
			->where('a.id_pegawai', $this->session->userdata('tpp_user_id'));
	...
	public function indexJson()
	{
		...
		$idfilter = decrypt_url($filter, 'standar');
		...
		$this->datatables->select('a.id, a.id_pegawai, a.bbebankerja, a.bkondisikerja, a.bkelangkaan, a.totaltpp, a.potongan_disiplin, a.potongan_produktivitas, a.hasiltpp, b.nama, b.nip, (c.disiplin_kerja/100 * a.totaltpp) as disiplin_kerja, (c.produktivitas_kerja/100 * a.totaltpp) as produktivitas_kerja, a.potonganpph, a.potonganbpjs, a.setelahpotongpph, a.jml_setelah_potongbpjs, a.potonganzakat, a.jml_setelah_potongzakat')
			...
			->where('b.att_status', 1)
			->where('b.dept_id', $dept_id)
			->where('a.update_by', $this->session->userdata('tpp_user_id'));
			if ($idfilter != null) {
				$this->datatables->where('a.tpp_standar',$idfilter);
			}
			$this->datatables->order_by('a.id', 'asc')
			->add_column('pegawai2', '<div class="m-0 p-1 panel-geser" style="max-height: 100px;border:none;">$1</div>', 'nama_icon_nip(nama,nip)')
			->add_column('bbebankerja', '<div class="m-0 p-1 panel-geser" style="max-height: 100px;border:none; ">$1</div>', 'format_rupiah(bbebankerja)')
	...
	public function AjaxSave()	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if ($golongan_id == '1' || $golongan_id == '2' || $golongan_id == '3' || $golongan_id == '4' || $golongan_id == '5') {
				$potonganpph = $hasil_tpp * 0.15;
			} elseif ($golongan_id == '6' || $golongan_id == '7' || $golongan_id == '8' || $golongan_id == '9') {
				$potonganpph = $hasil_tpp * 0.05;
			} else {
				$potonganpph = 0;
			}

			$setelahpotong_pph = $hasil_tpp - $potonganpph;
			
			$BPJS = 0.01;

			$PotonganBPJS = $setelahpotong_pph * $BPJS;

			$setelahpotong_bpjs = $setelahpotong_pph - $PotonganBPJS;
			...
			if ($golongan_id == '1' || $golongan_id == '2' || $golongan_id == '3' || $golongan_id == '4' || $golongan_id == '5' || $golongan_id == '6' || $golongan_id == '7' || $golongan_id == '8' || $golongan_id == '9' ) {
				$potongan_zakat = $setelahpotong_bpjs * 0.025;
			} else {
				$potongan_zakat = 0;
			}

			$setelahpotong_zakat = $setelahpotong_bpjs - $potongan_zakat;
	...
	public function cetak($bulan_id)
	{
		...
		$this->data['data_tpp']		= $this->m_tpp->getTppCetakIndividu($bulan_id, $dept_id)->result();
		...
		$this->load->view('tpp_individu/v_cetak', $this->data);
	}
...
/* End of file Tpp_Individu.php */
/* Location: ./application/modules/Kepegawaian/controllers/Tpp_Individu.php */
