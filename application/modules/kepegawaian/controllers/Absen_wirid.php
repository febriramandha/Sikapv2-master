...
class Absen_wirid extends App_Controller {
	...
	public function index()
	{
		...
		$this->load->view('absen_wirid/v_index', $this->data);
	}
	...
	public function indexJson()
	{
		...
		$this->datatables->select('id, tgl_apel, start_time, ket')
			->from('sch_apel')
			->where('jenis_apel_id',2)
			->order_by('tgl_apel','desc')
			->add_column('tanggal', '$1 ($2)', 'tgl_ind_hari(tgl_apel),jm(start_time)')
			->add_column('sch_name','$1','sch_name(ket, tgl_apel)')
			->add_column('action', '<a href="'.base_url('kepegawaian/absen-wirid/edit/').'$1" class="btn btn-sm badge-info p-1"> 
			<i class="icon-alarm-check"></i> 
			Input Absen Wirid</a>
			</span>', 'encrypt_url(id,"schapel_id")');
		return $this->output->set_output($this->datatables->generate());acktion_sch_status();
	}

	public function edit($id)
	{
		$this->data['sub_title'] 	= "Absen Wirid Pegawai";
		$this->breadcrumbs->push('Absen Wirid Pegawai', '/');
		$this->data['breadcrumb'] 	= $this->breadcrumbs->show();
		...
		$this->data['jadwal']			= $this->db->get_where('sch_apel',['id' => decrypt_url($id,'schapel_id')])->row();
		$this->load->view('absen_wirid/v_edit', $this->data);
	}
	...
	public function PegawaiJson($id)
	{
		...
		$this->output->unset_template();
		$schapel_id = decrypt_url($id,'schapel_id');
		...
		$this->load->library('datatables');
		$this->datatables->select('a.id, a.nip, a.nama,status_pegawai,gelar_dpn,gelar_blk,b.golongan, b.pangkat, a.eselon_id, c.hadir, c.id as absenapel_id')
			->from('v_users_all a')
			->join('_golongan b','a.golongan_id=b.id','left')
			->join("(select id, user_id, hadir from apel_pegawai where sch_apel_id=$schapel_id) c",'a.id=c.user_id','left')
			->where('key > 0')
			->where('att_status',1)
			->order_by('no_urut')
			->add_column('nama_gelar','$1','nama_gelar(nama, gelar_dpn,gelar_blk)')
			->add_column('golongan','$1/$2','golongan,pangkat')
			->add_column('cek1','$1',"cek_radio_disable(id,$eselon_,eselon_id,'yahadir',hadir,absenapel_id)")
			...
			->add_column('action','$1',"action_upacara_id(absenapel_id,$eselon_,eselon_id,hadir)");
		if ($instansi) {
	...
	public function AjaxSave($id)
	{
		...
		if ($this->form_validation->run() == TRUE) {
			$instansi = decrypt_url($this->input->post('instansi'),'instansi');
			$schupacara_id = decrypt_url($id,'schapel_id');
			...
			if ($data_update) {
				$this->return = $this->db->update_batch('apel_pegawai', $data_update, 'id');
			}

			if ($data_insert) {
				$this->return = $this->db->insert_batch('apel_pegawai', $data_insert);
			}
	...
	{
		$this->output->unset_template();
		$this->del = $this->db->delete('apel_pegawai',['id' => decrypt_url($this->input->get('id'),"absenupacara_id")]);
	...
	public function cetak($dept_id,$schupacara_id)
	{
		...
		$this->load->helper('absen');
		$schupacara_id = decrypt_url($schupacara_id,'schapel_id');
		...
		$this->db->select('a.id, a.nip, a.nama,status_pegawai,gelar_dpn,gelar_blk,b.golongan, b.pangkat, a.eselon_id, c.hadir, c.id as absenupacara_id')
			->from('v_users_all a')
			->join('_golongan b','a.golongan_id=b.id','left')
			->join("(select id, user_id, hadir from apel_pegawai where sch_apel_id=$schupacara_id) c",'a.id=c.user_id','left')
			->where('key > 0')
			->where('att_status',1)
			->order_by('no_urut')
			->where('a.dept_id', decrypt_url($dept_id, 'instansi'));
		$this->data['user_upacara']		= $this->db->get()->result();
		$this->data['jadwal']			= $this->db->get_where('sch_apel',['id' => $schupacara_id])->row();
		$this->load->view('absen_wirid/v_cetak', $this->data);
	}
...
/*End of file Absen_wirid.php */
/*Location: ./application/modules/Kepegawaian/controllers/Absen_wirid.php */
