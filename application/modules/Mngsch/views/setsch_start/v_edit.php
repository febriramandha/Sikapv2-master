...
<div class="card">
	<div class="card-header bg-white header-elements-inline py-2">
		<h5 class="card-title">Edit Jadwal Jam Kerja</h5>
  ...
  <?php echo form_open('mngsch/setsch-start/AjaxSave','class="form-horizontal" id="formAjax"'); ?>
		<div class="col-lg-12">
          <div class="form-group row">
              ...
              <div class="col-lg-9">
                  <div class="form-group-feedback form-group-feedback-left">
                      ...
                      <input type="text" name="nama" class="form-control" placeholder="Nama Jadwal Kerja" value="<?php echo $sch_run->name ?>" >
                  </div>
              </div>
          </div>
          <div class="form-group row">
         ...
         <div class="form-group row">
          ...
          <?php foreach ($sch_run_deil as $row_run) { ?>
          <div class="form-group row">
              <label class="col-form-label col-lg-2"><?php echo $row_run->day_ind ?><span class="text-danger">*</span></label>
              <div class="col-lg-10">
                  <div class="form-group">
                       <select class="form-control select-search" name="h<?php echo $row_run->s_day ?>" >  
                              <option disabled="">Pilih Jam Kerja</option>
                              <?php foreach ($sch_class as $row) { ?>
                                <option value="<?php echo $row->id ?>"  <?php if ($row_run->class_id == $row->id) { echo "selected";} ?> ><?php echo $row->name ?> (<?php echo $row->start_time ?> - <?php echo $row->end_time ?>)</option>
                              <?php } ?>
                      </select> 
                  </div>
              </div>
          ...
          <?php } ?>
          <input type="hidden" name="id" value="<?php echo encrypt_url($sch_run->id,"schrun_umum")  ?>">
  ...
  var spinner = $('#spinner');
    $.ajax({
        ...
        success: function(res) {
            if (res.status == true) {
                bx_alert_successUpadate(res.message, 'mngsch/setsch-start');
            }else {
