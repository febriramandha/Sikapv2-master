                  ...
                  <div class="form-group">
                       <select class="form-control select-search" name="class" >  
                              ...
                              <?php foreach ($sch_class as $row) { ?>
                                <option value="<?php echo $row->id ?>" <?php if ($shift_run->class_id == $row->id) { echo "selected"; } ?>><?php echo $row->name ?> (<?php echo $row->start_time ?> - <?php echo $row->end_time ?>)</option>
                              <?php } ?>
                      </select> 
                  </div>
              </div>
          ...
          <div class="form-group row">
              ...
              <div class="col-lg-9">
                  <div class="form-group-feedback form-group-feedback-left">
                      ...
                      <input type="number" name="jumlah" class="form-control" placeholder="jumlah hari" value="<?php echo $shift_run->count_day ?>">
                  </div>
              </div>
          </div>
          <div class="form-group row">
              ...
              <div class="col-lg-10">
                  <div class="form-group-feedback form-group-feedback-left">
                      ...
                      <input type="text" name="kode" class="form-control" placeholder="Kode" value="<?php echo $shift_run->kd_shift ?>">
                  </div>
              </div>
          </div>
          <div class="form-group row">
              ...
              <div class="col-lg-10">
                  <div class="form-group-feedback form-group-feedback-left">
                      ...
                      <textarea name="ket" class="form-control" placeholder="detail" > <?php echo $shift_run->ket ?></textarea>
                  </div>
              </div>
          </div>
           <div class="form-group row">
                  ...
                  <div class="form-group">
                       <select class="form-control multiselect-clickable-groups" name="instansi[]" multiple="multiple" id="filter_list_dropdwn" data-fouc>
                             <?php foreach ($instansi as $row) { 
                                     $instansi_cek =  pg_to_array($shift_run->dept_id);
                                     $selected ='';
              ...
              <div class="input-group">
                <span class="input-group-prepend">
                  <span class="input-group-text">
                    <input type="checkbox" name="status" class="form-control-switchery" <?php if ($shift_run->status == 1) { echo "checked";} ?> data-fouc> 
                  </span>
                </span>
              </div>
            </div>
          </div>
          <input type="hidden" name="id" value="<?php echo $shift_run->id ?>">
  ...
  var spinner = $('#spinner');
    $.ajax({
        ...
        success: function(res) {
            if (res.status == true) {
                bx_alert_successUpadate(res.message, 'mngsch/setinst-shift');
            }else {
