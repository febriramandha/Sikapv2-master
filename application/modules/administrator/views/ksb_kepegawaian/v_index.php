...
<div class="card">
	<div class="card-header bg-white header-elements-inline py-2">
		<h5 class="card-title">Data Kasubag Kepegawaian</h5>
	...
	<div class="card-body">
		<div class="form-group row">
			<div class="text-left col-lg-12">
				<a href="<?php echo base_url('administrator/ksb-kepegawaian/add') ?>" class="btn btn-sm btn-info"><i class="icon-pen-plus mr-1"></i> Tambah Baru</a>
			</div>
		</div>
		<div class="form-group row">
...
<script type="text/javascript">
	$(document).ready(function(){
		table = $('#datatable').DataTable({ 
			...
			ajax: {
				url : uri_dasar+'administrator/ksb-kepegawaian/indexJson',
				type:"post",
				"data": function ( data ) {	
	...
	function confirmAksi(id) {
		$.ajax({
			type: 'get',
			url: uri_dasar+'administrator/ksb-kepegawaian/AjaxDel',
			data: {id:id},
			dataType : "JSON",
			error:function(){
				$('.table').unblock();
	...
	$('#cetak').click(function() {
		newWindow = window.open(uri_dasar + 'administrator/ksb-kepegawaian/cetak/'+$('[name="instansi"]').val(),"open",'height=600,width=800');
