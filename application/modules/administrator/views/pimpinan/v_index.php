...
<div class="card">
	<div class="card-header bg-white header-elements-inline py-2">
		<h5 class="card-title">Data Kepala OPD</h5>
...
<script type="text/javascript">
	$(document).ready(function(){
		table = $('#datatable').DataTable({ 
			...
			ajax: {
				url : uri_dasar+'administrator/pimpinan/indexJson',
				type:"post",
				"data": function ( data ) {	
	...
	function confirmAksi(id) {
		$.ajax({
			type: 'get',
			url: uri_dasar+'administrator/pimpinan/AjaxDel',
			data: {id:id},
			dataType : "JSON",
			error:function(){
				$('.table').unblock();
	...
	$('#cetak').click(function() {
		newWindow = window.open(uri_dasar + 'administrator/pimpinan/cetak/'+$('[name="instansi"]').val(),"open",'height=600,width=800');
