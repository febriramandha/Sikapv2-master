...
<div class="card">
  ...
  <div class="card-body">
    ...
    <?php echo form_open('master/struktur-jabatan/AjaxSave/'.$this->uri->segment(4),'class="form-horizontal" id="formAjax"'); ?>
    <div class="col-lg-12">
      ...
      <div class="col-lg-10">
        <div class="form-group">
          <select class="form-control select-search" name="instansi"> 
            <?php foreach ($instansi_select as $row) { ?>
              <option value="<?php echo encrypt_url($row->id,'instansi_id') ?>"><?php echo level_instansi($row->level,$row->path_order)." ".strtoupper($row->nama_instansi) ?></option>
            <?php } ?>
          </select> 
        </div>
      </div>
    ...
    <div class="form-group row jabatan_tingkat">
        <label class="col-form-label col-lg-2">Jabatan Tingkat <span class="text-danger">*</span>
        <i class="icon-spinner2 spinner" style="display: none" id="spinner_tingkat"></i>
        </label>
        <div class="col-lg-10">
          <div class="form-group">
...
function JabatanTingkat() {
  var instansi_id = $('[name="instansi"]').val();
  var result  = $('.result');
  var spinner = $('#spinner_tingkat');
  $.ajax({
    type: 'get',
    url: uri_dasar+'master/struktur-jabatan/AjaxGet',
    data: {mod:'JabatanTingkat',instansi_id:instansi_id},
    dataType : "html",
    error:function(){
      result.attr("disabled", false);
          spinner.hide();
